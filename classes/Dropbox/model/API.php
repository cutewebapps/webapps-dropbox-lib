...
class Dropbox_API
{
	 ...
	 */
	public function __construct(Dropbox_OAuth_Consumer_ConsumerAbstract $OAuth, $root = 'sandbox')
	{
		$this->OAuth = $OAuth;
		$this->setRoot($root);
	}
	...
	*/
	public function setRoot($root)
	{
		if($root !== 'sandbox' && $root !== 'dropbox'){
			throw new Dropbox_Exception("Expected a root of either 'dropbox' or 'sandbox', got '$root'");
		} else {
			$this->root = $root;
		}
	}
	...
	public function putFile($file, $filename = false, $path = '', $overwrite = true)
	{
		if(file_exists($file)){
			...
			throw new Dropbox_Exception('File exceeds 150MB upload limit');
		}

		// Throw an Exception if the file does not exist
		throw new Dropbox_Exception('Local file ' . $file . ' does not exist');
	}
	...
	public function getFile($file, $outFile = false, $revision = null)
	{
		...
		if($this->responseFormat !== 'php'){
			throw new Dropbox_Exception('This method only supports the `php` response format');
		}
		...
		if($outFile !== false){
			...
			if(!$handle = fopen($outFile, 'w')){
				throw new Dropbox_Exception("Unable to open file handle for $outFile");
			} else {
	...
	public function setResponseFormat($format)
	{
		...
		if(!in_array($format, array('php', 'json', 'jsonp'))){
			throw new Dropbox_Exception("Expected a format of php, json or jsonp, got '$format'");
		} else {
